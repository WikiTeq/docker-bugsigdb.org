...
```sh
git clone https://github.com/WikiTeq/docker-bugsigdb.org.git
cd docker-bugsigdb.org
copy a database dump to __intdb directory
copy images to `bugsigdb.org/_data/mediawiki/images` directory
docker-compose up
```
...
`CusstomSettings.php` contains settings for MediaWiki core and extensions, you should change the settings there.
...
Data like uploaded images and the database files stored in `_data` directory
